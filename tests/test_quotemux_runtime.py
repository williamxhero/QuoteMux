...
def test_daily_snapshot_coverage_accepts_source_marked_full_day_suspension(monkeypatch) -> None:
    active_frame = pd.DataFrame.from_records(
        [
            {"code": "000001", "is_suspended": False},
            {"code": "000002", "is_suspended": True},
        ]
    )
    monkeypatch.setattr("quotemux.stocks.load_stock_active_codes_frame", lambda trade_date: active_frame)
    items = [
