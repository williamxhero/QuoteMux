...
class MemoryRowRepository:
    ...
    def read(self, capability_id: str, time_start: datetime, time_end: datetime, never_expires: bool, identity_prefix: str = "") -> tuple[dict[str, object], ...]:
        now = datetime.now()
        return tuple(
            payload
            for (stored_capability_id, time_key, identity_value), (payload, fresh_until) in self.items.items()
            if stored_capability_id == capability_id
            and time_start <= time_key <= time_end
            and (never_expires or fresh_until > now)
            and (identity_prefix == "" or identity_value == identity_prefix or identity_value.startswith(identity_prefix + "|"))
        )
...
def _patch_store_context(monkeypatch, store: UnifiedPostgresCacheStore) -> None:
    ...
    monkeypatch.setattr("quotemux.stocks.load_store_result", load_from_store)
    monkeypatch.setattr("quotemux.stocks.store_result", write_to_store)
    monkeypatch.setattr("quotemux.store.load_store_result", load_from_store)
    monkeypatch.setattr("quotemux.store.store_result", write_to_store)
...
def test_factor_cache_read_only_loads_the_requested_code_scope() -> None:
    store = _store(_policy_for("stocks.factors.adj"))
    report = ContractReport(contract_name="stocks.factors.adj")
    request = {"code": "000001", "start_date": "2026-04-03", "end_date": "2026-04-03", "base_date": ""}

    store.write(
        "stocks.factors.adj",
        request,
        [AdjFactorItem(code="000001", trade_date="20260403", adj_factor=1.0)],
        report,
    )
    store.write(
        "stocks.factors.adj",
        {**request, "code": "000002"},
        [AdjFactorItem(code="000002", trade_date="20260403", adj_factor=2.0)],
        report,
    )

    result = store.read("stocks.factors.adj", request)
...
def test_main_business_cache_coverage_uses_report_period_payloads() -> None:
    store = _store(_policy_for("stocks.finance.main_business"))
    report = ContractReport(contract_name="stocks.finance.main_business")

    store.write(
        "stocks.finance.main_business",
        {"code": "000970", "report_period": "", "start_period": "", "end_period": "", "classification": "industry"},
        [
            MainBusinessItem(code="000970", report_period="2024-12-31", classification="industry", segment_name="其他业务"),
            MainBusinessItem(code="000970", report_period="2025-06-30", classification="industry", segment_name="其他业务"),
        ],
        report,
    )
    result = store.read(
...
def test_postgresql_cache_read_adds_escaped_identity_prefix(monkeypatch) -> None:
    captured: dict[str, object] = {}

    monkeypatch.setattr(postgres, "_ensure_schema", lambda: True)
    monkeypatch.setattr(postgres, "query_dataframe", lambda query, params: captured.update(query=query, params=params) or __import__("pandas").DataFrame())

    result = postgres.CacheRowRepository().read(
        "stocks.factors.adj",
        datetime(2026, 4, 3),
        datetime(2026, 4, 3),
        False,
    ...
    assert result == ()
    assert "identity_value = %s or identity_value like %s" in str(captured["query"])
    assert "escape '\\'" in str(captured["query"])
    assert captured["params"][-2:] == ("code=000001", "code=000001|%")
...
def test_default_policy_specs_cover_public_capabilities() -> None:
    ...
    disabled_ids = {item.capability_id for item in postgres.DEFAULT_POLICY_SPECS if not item.enabled}

    assert capability_ids == policy_ids
    assert disabled_ids == {"futures.quotes.contract.realtime", "futures.quotes.main_continuous.realtime"}


def test_derived_calendar_apis_read_root_trading_calendar_store(monkeypatch) -> None:
    settings = QuoteMuxSettings(enabled_sources=("derived_core", "tushare", "akshare"))
    ...
    monkeypatch.setattr(
        "quotemux.markets._source_package_call",
        _source_call_stub(
            {
                ("tushare", "get_trading_calendar"): AssertionError("source should not run"),
                ("akshare", "get_trading_calendar"): AssertionError("source should not run"),
                ...
                ("derived_core", "get_yearly_trading_calendar"): calendar_items,
            }
        ),
    )
...
def test_empty_daily_snapshot_does_not_write_store(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda spec: ([], ContractReport(contract_name=spec.capability_id).with_store_stats(miss=True)),
    )
...
def test_remaining_capability_families_store_roundtrip(monkeypatch) -> None:
    ...
    monkeypatch.setattr("quotemux.news._source_package_call", lambda *args: [])
