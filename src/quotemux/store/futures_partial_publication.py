    ...
    def publish(self, *, qmi_id: str, catalog_identity: str, expected_generation: int | None = None) -> dict[str, str]:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                select %s, 'qmb-v1-'||md5(product_code||'|'||exchange||'|'||source_key||'|'||min(bar_time)::text||'|'||max(bar_time)::text), product_code,exchange,series_type,source_key,min(bar_time),max(bar_time),jsonb_build_object('meaning','actual fact source ownership')
                from fact.future_bar_1m where series_type='apex_l0_adjusted' group by product_code,exchange,series_type,source_key on conflict do nothing""", (qmp_id,))
