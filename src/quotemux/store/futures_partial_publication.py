        ...
        try:
            with connection.cursor() as cursor:
                ...
                with changes as (select product_code,exchange,series_type,source_key,bar_time,
                   case when lag(source_key) over (partition by product_code,exchange,series_type order by bar_time) is not distinct from source_key then 0 else 1 end as changed from fact.future_bar_1m where series_type='apex_l0_adjusted' and (product_code,bar_time) not in (('TA','2010-11-17 09:05:00'::timestamp),('y','2010-11-11 14:01:00'::timestamp),('y','2011-02-23 11:22:00'::timestamp))), ordered as
