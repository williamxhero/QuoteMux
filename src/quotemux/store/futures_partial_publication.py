        ...
        try:
            with connection.cursor() as cursor:
                ...
                with changes as (select product_code,exchange,series_type,source_key,bar_time,
                   case when lag(source_key) over (partition by product_code,exchange,series_type order by bar_time) is not distinct from source_key then 0 else 1 end as changed from fact.future_bar_1m where series_type='apex_l0_adjusted'), ordered as
