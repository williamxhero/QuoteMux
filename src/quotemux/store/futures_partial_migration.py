...
METADATA_DDL = (
    ...
    """create table if not exists audit.future_bar_1m_partial_publication (qmp_id text primary key check(qmp_id ~ '^qmp-v1-[0-9a-f]{64}$'),dataset_id text not null,payload_json jsonb not null,payload_sha256 text not null,published_at timestamptz not null default now())""",
    ...
    """create table if not exists audit.future_bar_1m_partial_revision (qmc_id text primary key check(qmc_id ~ '^qmc-v1-[0-9a-f]{64}$'),qmp_id text not null references audit.future_bar_1m_partial_publication(qmp_id),payload_json jsonb not null,payload_sha256 text not null,published_at timestamptz not null default now())""",
