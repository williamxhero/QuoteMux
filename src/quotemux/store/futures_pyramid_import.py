...
class FuturesPyramidImporter:
    ...
    def publish_filesystem_bundle(self, bundle_path: Path, plan_path: Path) -> dict[str, object]:
        ...
        manifest_sha = hashlib.sha256(canonical_json_bytes(bundle.manifest)).hexdigest()
        if header.get("bundle_manifest_sha256") != manifest_sha or header.get("bundle_source_normalized_rowset_sha256") != bundle.source_normalized_rowset_sha256 or header.get("bundle_canonical_fact_rowset_sha256") != bundle.canonical_fact_rowset_sha256 or header.get("fact_transform_version") != FACT_TRANSFORM_VERSION: raise FuturesPyramidImportError("plan does not bind this exact verified bundle")
            ...
            with connection.cursor(row_factory=tuple_row) as cursor:
                ...
                try:
                    while evidence_rows := evidence_cursor.fetchmany(100_000):
                        for planned, expected_source, expected_sha, product, exchange, bar_time, open_, high, low, close, volume, offset, oi, source in evidence_rows:
                            if product is None: raise FuturesPyramidImportError("planned existing disposition disappeared")
                            actual_payload={"product_code":product,"exchange":exchange,"bar_time":str(bar_time),"open":open_,"high":high,"low":low,"close":close,"volume":volume,"adjustment_offset":offset,"open_interest":oi,"series_type":SERIES_TYPE,"source_key":source}
                            if source != expected_source or hashlib.sha256(canonical_json_bytes(actual_payload)).hexdigest() != expected_sha: raise FuturesPyramidImportError("planned existing fact evidence is stale")
                finally: evidence_cursor.close()
