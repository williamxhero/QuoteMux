...
class QuoteMuxRankings:
    ...
    def get_research_reports(self, trade_date: str, start_date: str, end_date: str, limit: int) -> list[RankingResearchReportItem]:
        if not self._settings.is_source_enabled("tushare"):
            return []
        return _tushare_provider.get_rank_research_reports(trade_date, start_date, end_date, ensure_limit(limit))

    def get_broker_monthly_picks(self, trade_month: str, limit: int) -> list[RankingBrokerPickItem]:
        if not self._settings.is_source_enabled("tushare"):
            return []
        return _tushare_provider.get_rank_broker_monthly_picks(trade_month, ensure_limit(limit))
