...
def _expected_trade_dates(start_date: date, end_date: date) -> set[date]:
    items = _tushare_provider.get_trading_calendar("SSE", _trade_date_text(start_date), _trade_date_text(end_date), True)
...
class QuoteMuxDatasets:
    ...
    def get_stock_bar_1m(self, request: StockBar1mRequest, base_df: pd.DataFrame) -> tuple[pd.DataFrame, ContractReport]:
        ...
        handlers = {
            "get_stock_quotes": lambda instance: lambda codes, start_text, end_text: {
                "opentdx": _opentdx_provider,
                "efinance": _efinance_provider,
                "mootdx": _mootdx_provider,
                "akshare": _akshare_provider,
            }[instance.package_id].get_stock_quotes(codes, "1m", "", start_text, end_text, "", "", None, "none"),
        }
    ...
    def get_index_bar_1d(self, request: IndexBar1dRequest, base_df: pd.DataFrame) -> tuple[pd.DataFrame, ContractReport]:
        ...
        handlers = {
            "get_index_quotes": lambda instance: lambda index_codes, start_text, end_text: {
                "opentdx": _opentdx_provider,
                "efinance": _efinance_provider,
                "mootdx": _mootdx_provider,
                "akshare": _akshare_provider,
            }[instance.package_id].get_index_quotes(index_codes, "1d", "", start_text, end_text, None),
        }
        merged_items, fallback_report = run_fallback_chain_with_report(
    ...
    def repair_stock_daily_ohlcva(self, request: StockDailyOhlcvaRepairRequest, df_day: pd.DataFrame) -> tuple[pd.DataFrame, ContractReport]:
        ...
        handlers = {
            "get_stock_quotes": lambda instance: lambda codes, start_text, end_text: {
                ...
                "mootdx": _mootdx_provider,
                "akshare": _akshare_provider,
            }[instance.package_id].get_stock_quotes(codes, "1d", start_text, "", "", "", "", None, "none"),
        }
        merged_items, fallback_report = run_fallback_chain_with_report(
